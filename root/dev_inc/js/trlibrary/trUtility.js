...
/**
 * trNumReturn		: 숫자 값이 있는지를 판단하여 값이없으면 기본값(0)을 리턴한다.
 * @param $val		: 숫자 값변수	   (필수)
 * @param $default  : 기본값 세팅	   (선택)
 */
function trNumReturn ($val, $default){
	if ($val == undefined){
		$val = 0;
		if ($default)
		{
...
/**
 * trStrReturn		: 문자 값이 있는지를 판단하여 값이없으면 기본값("")을 리턴한다.
 * @param $val		: 문자 값변수   (필수)
 * @param $default  : 기본값 세팅   (선택)
 */
function trStrReturn ($val, $default){
	if ($val == undefined){
		$val = "";
		if ($default)
		{
...
/* 
 * date : 20141105
 * trBrowser		: 브라우저 정보를 리턴함.
 * @method name		: 브라우저 이름
 * @method version	: 브라우저 버젼
*/ 

var trBrowser = (function() {
...
/**
* trNumberFormat	: 숫자를 넣으면 3자리수마다 "," 있는 문자열을 리턴
* @param $num		: 문자 값변수   (필수)
*/
function trNumberFormat($num) {
 ...
 */
function trRangeRatio($rMin, $rMax, $rTar, $tMin, $tMax ){
